...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['search_user'])) {
    ...
    if (empty($email)||empty($username)) {
        ...
        if ($result->num_rows === 1) {
            ...
            $_SESSION['fetchedUserId'] = $user['id'];
            $message = "User found. You can now enter the amount to fund.";
            $userFound = true;
        } elseif ($result->num_rows > 1) {
            $message = "Multiple users found with the same details. Please refine your search.";
            $userFound = false;
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fund_user'])) {
    ...
    $fetchedUserId = $_SESSION['fetchedUserId'] ?? null;

    // Validate inputs
    if ($fetchedUserId === null) {
        $message = "No user selected for funding.";
    } elseif (!is_numeric($amount) || $amount <= 0) {
        $message = "Invalid amount.";
    } else {
        // Update user's balance
        $stmt = $conn->prepare("UPDATE users SET balance = balance + ? WHERE id = ?");
        $stmt->bind_param("di", $amount, $fetchedUserId);

        if ($stmt->execute()) {
            $message = "User funded successfully.";
        } else {
            $message = "Failed to fund user.";
        }
  ...
  <body>
    ...
    <div class="card mb-4">
        <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="mb-0">Enter User Details to Fund User</h5>
        </div>
        <div class="card-body">
            <form method="POST" action="">
                ...
                <!-- Amount (visible if user is found) -->
                <?php if ($userFound): ?>
                    <div class="row mb-3">
                ...
                <div class="row justify-content-end">
                    <div>
                        <button
                            ...
                            name="search_user">Search User</button>
                        <?php if ($userFound): ?>
                            <button
                                type="submit"
                                class="btn btn-success"
                                name="fund_user">Fund</button>
                        <?php endif; ?>
                    </div>
                </div>
            </form>
